...
public class UserJpaRepositoryIT {
	...
	/*
	 * @Test public void shouldReturn400StatusCodeForInvalidUsername() throws
	 * Exception { user.setUsername(null);
	 * mvc.perform(post("/users").content(gson.toJson(user)).contentType(
	 * MediaType.APPLICATION_JSON)) .andExpect(status().is4xxClientError()); }
	 * 
	 * @Test public void shouldReturn400StatusCauseUsernameLengthIsLessThen5()
	 * throws Exception { user.setUsername("abcd");
	 * mvc.perform(post("/users").content(gson.toJson(user)).contentType(
	 * MediaType.APPLICATION_JSON)) .andExpect(status().is4xxClientError()); }
	 * 
	 * @Test public void shouldReturn400StatusCauseUsernameLengthIsMoreThen20()
	 * throws Exception { user.setUsername("123456789012345678901");
	 * mvc.perform(post("/users").content(gson.toJson(user)).contentType(
	 * MediaType.APPLICATION_JSON)) .andExpect(status().is4xxClientError()); }
	 * 
	 * @Test public void shouldReturn400StatusCodeForInvalidEmail() throws
	 * Exception { user.setEmail("abcdefg");
	 * mvc.perform(post("/users").content(gson.toJson(user)).contentType(
	 * MediaType.APPLICATION_JSON)) .andExpect(status().is4xxClientError()); }
	 * 
	 * @Test public void shouldReturn400StatusCodeForEmptyEmail() throws
	 * Exception { user.setEmail(null);
	 * mvc.perform(post("/users").content(gson.toJson(user)).contentType(
	 * MediaType.APPLICATION_JSON)) .andExpect(status().is4xxClientError()); }
	 * 
	 * @Test public void shouldReturn400StatusCauseEmailLengthIsLessThen5()
	 * throws Exception { user.setEmail("a@as");
	 * mvc.perform(post("/users").content(gson.toJson(user)).contentType(
	 * MediaType.APPLICATION_JSON)) .andExpect(status().is4xxClientError()); }
	 * 
	 * @Test public void shouldReturn400StatusCauseEamilLengthIsMoreThen30()
	 * throws Exception { user.setEmail("12345678@012345678901.345678901");
	 * mvc.perform(post("/users").content(gson.toJson(user)).contentType(
	 * MediaType.APPLICATION_JSON)) .andExpect(status().is4xxClientError()); }
	 * 
	 * @Test public void shouldReturn400StatusCodeForEmptyPassword() throws
	 * Exception { user.setPassword(null);
	 * mvc.perform(post("/users").content(gson.toJson(user)).contentType(
	 * MediaType.APPLICATION_JSON)) .andExpect(status().is4xxClientError()); }
	 * 
	 * // End of Unit test for User constraint violations
	 * 
	 * @Test public void shouldReturnSingleUserObject() throws Exception {
	 * mvc.perform(post("/users").content(gson.toJson(user))).andExpect(status()
	 * .isCreated());
	 * mvc.perform(get("/users")).andExpect(status().isOk()).andExpect(jsonPath(
	 * "$._embedded.users", hasSize(1))) .andDo(document("users",
	 * links(linkWithRel("self").description("Link to user resource"),
	 * linkWithRel("profile").description("Link to user profile"),
	 * linkWithRel("search").description(
	 * "Link for search method for user resource")), responseFields(
	 * 
